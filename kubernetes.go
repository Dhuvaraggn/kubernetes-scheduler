...
func fit(pod *Pod) ([]Node, error) {
	...
	for _, node := range nodeList.Items {
		...
		if strings.HasSuffix(node.Status.Allocatable["cpu"], "m") {
		    log.Println(node.Status.Allocatable["memory"])
...
func bind(pod *Pod, node Node) error {
    log.Println(pod.Metadata.Name+" scheduling on "+node.Metadata.Name)
	binding := Binding{
		ApiVersion: "v1",
		Kind:       "Binding",
